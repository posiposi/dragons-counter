...
_assign-ports:
	@OFFSET=$$(echo "$(CURRENT_DIR)" | cksum | awk '{print ($$1 % 50 + 1) * 100}'); \
	grep -v '^HOST_.*_PORT=' .env > .env.tmp && mv .env.tmp .env; \
	grep -v '^# Worktree dynamic port' .env > .env.tmp && mv .env.tmp .env; \
	{ \
		echo ""; \

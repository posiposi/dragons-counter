...
> **Agent Teams使用規約:** Agent Teamsは**複雑かつ大規模なタスク**に対してのみ使用する。小規模な場合は基本の確認手順（メインコンテキスト）で確認する。なお、agent teamの起動にユーザーの許可は不要。
...
複雑・大規模な場合、コンフリクト確認を**複数のagent teamsで並列実施**して調査の網羅性と効率を高める。以下の2段構成で実施する。
...
> **注意:** `git worktree add` / `git commit` / `git push` 等のgitコマンド実行にユーザーの許可は不要。

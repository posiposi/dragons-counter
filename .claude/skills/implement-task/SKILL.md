...
Phase 2-cで分解した**1コミット粒度の実装タスクごと**に、メインコンテキストが以下の6ステップをオーケストレーションする。

**オーケストレーションの所在（重要）**: サブエージェントはスラッシュコマンド（`/commit-commands:commit` 等）を実行できない。したがって、`/commit-commands:commit` によるコミットは**メインコンテキストが担う**。実装作業（テストコード作成・実装・テスト実行）は `implementer` サブエージェントに委任する。実装時のTDDサイクル・lint確認は `implementer` がプリロードするスキル（`tdd-workflow`、`typescript-ddd-standards`、`linter-execute`）に従う。

各タスクについて、以下の手順を実行する：

1. **テストコード作成**: `implementer` を起動し、対象タスクのテストコード（`*.spec.ts`）を作成する
2. **RED確認**: `implementer` を起動し、テストを実行して**失敗（RED）すること**を確認する
3. **コード実装**: `implementer` を起動し、テストが通る最小限の実装を行う
4. **GREEN確認**: `implementer` を起動し、テストを実行して**成功（GREEN）すること**を確認する
5. **コミット**: メインコンテキストが `/commit-commands:commit` を使用してコミットする（実行許可の確認は不要）。1コミットの粒度は、そのコミット単体で意味を把握できる単位に限定する（判断しにくい大きな粒度でのコミットは禁止）
6. **繰り返し**: 未消化の実装タスクが残っている場合、1. に戻り次のタスクを実装する

全タスクの実装が完了したら、**Phase 4（PR作成）** へ移行する。

**補足**: コミット単位ごとの個別レビュー（旧 `pr-review-toolkit:*` エージェントによる並列レビュー）は、実装フローが重くなるため廃止した。コードレビューはPhase 4-3でPR全体に対して `/code-review:code-review` を用いて一括実行し、指摘修正はPhase 4-4で行う。`/code-review:code-review` はPR（`gh pr`）を前提とするため、PR作成前のコミット単位では実行しない。
...
[Phase 1] 仕様取得 (metadata: issue_spec)
  ...
  → [Phase 2-c] タスク分解 (blockedBy: 2-a, 2-b)
    → [Phase 3] 実装ループ (blockedBy: 2-c) ※タスクごとに6ステップ（TDD実装 + コミット）を反復
      → [Phase 4] PR作成・レビュー・指摘修正
```
...
- レビュー（`/code-review:code-review`）の実行、および `/commit-commands:commit` によるコミットは、すべてメインコンテキストが行う。サブエージェントはスラッシュコマンドおよびAgentツール（他サブエージェントの起動）を使用できないため、オーケストレーションはサブエージェントに委ねない
- コードレビューはPhase 4-3でPR全体に対して `/code-review:code-review` を用いて実行する。コミット単位の個別レビュー（`pr-review-toolkit:*`）は行わない

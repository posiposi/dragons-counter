...
テストは**自前で実装したロジック（自プロジェクトのコード）の振る舞い**を検証対象とする。ライブラリや言語機能そのものの挙動を確認するだけのテストは作成しない。これらは提供元で保証されており、記述してもリグレッション防止に寄与せず保守コストのみが増えるため不要とする。

**不要なテスト（作成しない）の例**

- ライブラリのラッパー関数で、実質的にライブラリの出力・パース・変換結果だけを検証するもの（例: DSN生成関数がドライバの整形結果と一致することの確認）
- ライブラリの標準機能に委ねている挙動の確認（例: env読込ライブラリが環境変数を構造体フィールドへマッピングすること、`envDefault`によるデフォルト値付与）
- 言語・コンパイラ機能そのものの確認（例: `//go:embed` でファイルが埋め込まれること）
- 外部ライブラリが接続失敗時にエラーを返すことだけを確認するもの

**必要なテスト（作成する）の例**

- 分岐・変換・判定など自前の制御ロジック（例: 環境変数値に応じた実行モード・アドレス・TLS設定の決定）
- 自前のハンドラ・ルーティング・レスポンス仕様
- 実データ・実DBに対する統合的な振る舞い（例: マイグレーション適用後のスキーマ・制約名の一致検証）

判断基準は「そのテストが落ちるのは自分たちのコードに不具合があるときか、それともライブラリ側の仕様が変わったときか」。後者のみを検出するテストは不要とする。

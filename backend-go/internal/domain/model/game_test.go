...
type gameFixture struct {
	...
	dragonsScore  model.Score
	opponentScore model.Score
...
func newGameFixture(t *testing.T, dragonsScore, opponentScore int) gameFixture {
	...
	gameDate, err := model.NewGameDate(time.Date(2024, 4, 1, 0, 0, 0, 0, time.UTC))
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}

	opponent, err := model.NewOpponent("阪神タイガース")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}

	dScore, err := model.NewScore(dragonsScore)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}

	oScore, err := model.NewScore(opponentScore)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}

	stadiumName, err := model.NewStadiumName("バンテリンドーム ナゴヤ")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	...
	return gameFixture{
		...
		stadium:       model.NewStadium(model.NewStadiumID(), stadiumName),
...
func TestNewGame(t *testing.T) {
	t.Run("有効な値でGameを生成し各フィールドをゲッターで取得できる", func(t *testing.T) {
		...
		if g.Result().Value() != model.Win {
			t.Errorf("Game.Result().Value() = %v, want %v", g.Result().Value(), model.Win)
		}
	})
}
...
func TestNewGame_ResultDetermination(t *testing.T) {
	tests := []struct {
		...
		{name: "ドラゴンズの得点が多い場合はresultがWINになる", dragonsScore: 7, opponentScore: 3, expected: model.Win},
		{name: "相手の得点が多い場合はresultがLOSEになる", dragonsScore: 2, opponentScore: 5, expected: model.Lose},
		{name: "同点の場合はresultがDRAWになる", dragonsScore: 4, opponentScore: 4, expected: model.Draw},
	}

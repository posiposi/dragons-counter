...
func newTestIDs(t *testing.T) (model.ID, model.ID) {
	t.Helper()
	userID, err := model.ParseID("user-1")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	gameID, err := model.ParseID("game-1")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func newTestImpression(t *testing.T, value string) model.Impression {
	t.Helper()
	impression, err := model.NewImpression(&value)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	return impression
...
func newEmptyImpression(t *testing.T) model.Impression {
	t.Helper()
	impression, err := model.NewImpression(nil)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	return impression
...
func TestCreateNewUserGame(t *testing.T) {
	t.Run("指定したuserIDとgameIDで作成できる", func(t *testing.T) {
		...
		userGame := model.CreateNewUserGame(userID, gameID, newEmptyImpression(t))
	...
	t.Run("Impressionありで作成できる", func(t *testing.T) {
		...
		userGame := model.CreateNewUserGame(userID, gameID, impression)
...
func TestUserGameFromRepository(t *testing.T) {
	t.Run("すべてのフィールドを復元できる", func(t *testing.T) {
		id := model.NewUserGameID()
		userID, gameID := newTestIDs(t)
		impression := newTestImpression(t, "良い試合")
		createdAt := time.Date(2026, 1, 1, 10, 0, 0, 0, time.UTC)
		updatedAt := time.Date(2026, 1, 2, 12, 30, 0, 0, time.UTC)

		userGame := model.UserGameFromRepository(id, userID, gameID, impression, createdAt, updatedAt)
...
func TestUserGameEquals(t *testing.T) {
	t.Run("IDが同一なら他フィールドが異なっても等価である", func(t *testing.T) {
		id := model.NewUserGameID()
		userID, gameID := newTestIDs(t)
		otherUserID, err := model.ParseID("user-2")
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		createdAt := time.Date(2026, 1, 1, 10, 0, 0, 0, time.UTC)

		userGame1 := model.UserGameFromRepository(
			id, userID, gameID, newTestImpression(t, "良い試合"), createdAt, createdAt,
		)
		userGame2 := model.UserGameFromRepository(
	...
	t.Run("IDが異なると非等価である", func(t *testing.T) {
		...
		userGame1 := model.CreateNewUserGame(userID, gameID, impression)
		userGame2 := model.CreateNewUserGame(userID, gameID, impression)
...
func TestUserGameUpdateImpression(t *testing.T) {
	t.Run("新しいImpressionを持つ新インスタンスを返す", func(t *testing.T) {
		userID, gameID := newTestIDs(t)
		original := model.CreateNewUserGame(userID, gameID, newTestImpression(t, "最初の感想"))
		newImp := newTestImpression(t, "更新後の感想")

		updated := original.UpdateImpression(newImp)

		if !updated.Impression().Equals(newImp) {
	...
	t.Run("元のインスタンスのImpressionは変更されない", func(t *testing.T) {
		...
		original := model.CreateNewUserGame(userID, gameID, originalImpression)
	...
	t.Run("更新後もIDが維持される", func(t *testing.T) {
		userID, gameID := newTestIDs(t)
		original := model.CreateNewUserGame(userID, gameID, newEmptyImpression(t))
